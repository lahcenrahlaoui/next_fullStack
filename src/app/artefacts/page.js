...
const ItemForm = () => {
    ...
    const randRRandom = (r) =>{
        ...
        for(let i = 0 ; i<r ; i++){
            ...
            x.push(randomTexts(y))
        }
        return x.join(" ")
    }

    const randomName = randRRandom(4)
    const randomDesc = randRRandom(40)

    const [item, setItem] = useState({
        id: randomTexts(2)+randomTexts(3),
        name: randomName,
    ...
    const handleImageChange = (e) => {
        ...
        reader.onloadend = () => {
            setImage(reader.result); // Save the file object for display
        };
    ...
    const handleThreeDItemChange = (e) => {
        ...
        const reader = new FileReader();
        reader.onloadend = () => {
            setThreeDItem(reader.result); // Save the file object for display
        };
        reader.readAsDataURL(file);
    };
    ...
    const handleUploadThreeD = async () => {
        ...
        const data = await response.json();
        console.log("Success 3d:", data);
        return data.map(({secure_url})=>secure_url);
    };
